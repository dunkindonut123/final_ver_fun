...
5. Run LMS v2 migrations in order (existing databases only):
   ...
   - Then: [supabase/migration_v2_chapter_count_by_hsk.sql](supabase/migration_v2_chapter_count_by_hsk.sql) — syncs chapter count per HSK (counts live in `lib/lms/hsk-chapters.ts`; re-run this SQL after changing them, then re-run `migration_v2_assignment_a_count_by_hsk.sql` so new chapters get assignment slots)
...
### Vercel shows `MIDDLEWARE_INVOCATION_FAILED`
- Add `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, and `SUPABASE_SERVICE_ROLE_KEY` in Vercel → Settings → Environment Variables
